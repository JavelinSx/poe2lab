...
> **Игрокам здесь ничего делать не нужно.** Сервер уже работает, и его адрес прописан в приложении
> (`RELAY_URL` в `poe2lab/feedback.py`): после `git clone` или скачивания ZIP форма сразу шлёт отзывы автору.
> Эта папка нужна только владельцу сервера, а ещё тому, кто сделал форк и хочет получать отзывы к себе: он
> разворачивает свой Worker по инструкции ниже и меняет `RELAY_URL`.
