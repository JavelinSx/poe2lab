...
def pair():
    ...
    ref, ref_bp = open_build(BUILDS / "ma95.txt")
...
def test_foreign_weapon_try_on_is_fully_undone(pair):
    ...
    # a quarterstaff cannot use the Titan's mace skills: PoB moves socket groups to the other weapon set
    with mine.swapped_items({"Weapon 1": ref.item_text("Weapon 1"), "Belt": None}):
...
def test_versus_lists_stats_and_slots(pair):
    mine, profile, ref, ref_profile = pair
    v = versus(mine, profile, ref, ref_profile)
    rows = {r["key"]: r for r in v["rows"]}
    assert rows["es"]["mine"] == 0 and rows["es"]["ref"] > 4000  # the Monk stands on energy shield
    assert {"hit_Physical", "res_Chaos", "Str", "dps"} <= rows.keys()
    slots = {s["slot"]: s for s in v["slots"]}
    assert slots["Weapon 1"]["swap"]["dps_pct"] <= -99  # their quarterstaff does not fit a mace skill
    assert slots["Ring 1"]["swap"] is not None and slots["Ring 1"]["ref"]["name"]
    assert v["allGear"]["es"] > 0
