...
const I18N = {
  ru: {
    ...
    jnWeightsSub: "Расчёт начинает с допущения крафта (все семейства модов и все тиры одинаковы) и сдвигает его по записанным пробам. Пока проб мало, веса почти не меняются.",
    ...
    jnHalfLevel: (n) => `По пробам вес тира падает вдвое каждые ${n} уровней мода.`,
    jnNoFalloff: "Верхние тиры по пробам выпадают не реже нижних.",
    jnSeen: "выпало",
    ...
    crWeightsNote: "Веса модов PoE2 игра не показывает. По журналу крафта (первые 458 проб: броня, амулеты, кольца) тиры одного мода выпадают одинаково часто — верхний так же, как нижние, а сами моды различаются (здоровье, сопротивления — примерно в 2,5 раза чаще среднего). Здесь все тиры и моды считаются равными, поэтому шансы — оценка; «Применить к крафту» в журнале уточняет веса модов. «Невезение» — 9 из 10 игроков уложатся в эту цифру.",
  ...
  en: {
    ...
    jnWeightsSub: "The estimate starts from crafting's assumption (all mod families and all tiers alike) and moves it by the recorded draws. With few draws the weights hardly change.",
    ...
    jnHalfLevel: (n) => `By the draws, a tier's weight halves every ${n} mod levels.`,
    jnNoFalloff: "By the draws, high tiers roll no less often than low ones.",
    jnIlvl: (n) => `ilvl ${n}`,
    jnDrawsN: (n) => `+${n} draws`,
    ...
    crWeightsNote: "PoE2 does not show mod weights. By the craft journal (the first 458 draws: body armour, amulets, rings) the tiers of a mod roll equally often, the top one as often as the rest, while mods differ (life, resistances about 2.5x the average). Here all tiers and mods count alike, so the chances are estimates; \"Apply to crafting\" in the journal refines the mods' weights. \"Bad luck\": 9 of 10 players finish within it.",
