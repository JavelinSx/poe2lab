...
const I18N = {
  ru: {
    ...
    via: (list) => `по пути: ${list}`, treeGrowth: "Куда расти",
    ...
    hitsNote: "Число — не урон, который ты получаешь, а самый сильный удар монстра (базовое значение, до модификаторов карты), который ты переживёшь с полного здоровья. Чем меньше число, тем опаснее: на хард-карте монстры бьют сильнее и больнее критуют, поэтому убить может уже более слабый в базе удар. Полоска — доля от лучшего значения в таблице.",
    ...
    hitsSub: (p) => `С полного здоровья. Моб ${p.enemy_level} ур.; хард-карта: монстры наносят +${p.damage_pct}% урона и +${p.crit_bonus}% к бонусу крита.`,
  ...
  en: {
    ...
    via: (list) => `on the way: ${list}`, treeGrowth: "Where to grow",
    ...
    hitsNote: "The number is not damage you take but the largest monster hit (base value, before map mods) you survive from full life. Smaller is more dangerous: on a hard map monsters hit harder and crit harder, so a weaker base hit already kills. The bar is its share of the best value in the table.",
    ...
    hitsSub: (p) => `From full life. Level ${p.enemy_level} monster; hard map: monsters deal +${p.damage_pct}% damage and +${p.crit_bonus}% crit bonus.`,
