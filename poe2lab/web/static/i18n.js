...
const I18N = {
  ru: {
    ...
    jnWeightsSub: "Расчёт начинает с нынешнего допущения (все семейства модов одинаковы, вес тира падает вдвое на каждые 25 уровней) и сдвигает его по записанным пробам. Пока проб мало, веса почти не меняются.",
    ...
    jnHalfLevel: (n) => `Вес тира падает вдвое каждые ${n} уровней мода (было допущение: 25).`,
    jnNoFalloff: "Верхние тиры по пробам выпадают не реже нижних (было допущение: вес тира падает вдвое каждые 25 уровней).",
    jnSeen: "выпало",
  ...
  en: {
    ...
    jnWeightsSub: "The estimate starts from the current assumption (all mod families alike, a tier's weight halving every 25 levels) and moves it by the recorded draws. With few draws the weights hardly change.",
    ...
    jnHalfLevel: (n) => `A tier's weight halves every ${n} mod levels (the assumption was 25).`,
    jnNoFalloff: "By the draws, high tiers roll no less often than low ones (the assumption was a tier's weight halving every 25 levels).",
    jnIlvl: (n) => `ilvl ${n}`,
    jnDrawsN: (n) => `+${n} draws`,
