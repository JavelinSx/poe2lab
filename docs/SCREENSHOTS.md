...
- **Внизу слева** — «📖 словарь новичка» (термины игры своими словами), «⚒ журнал крафта» (статистика выпадения модов для настоящих весов, см. [руководство](../GUIDE.md)), «✉ сообщить о нестыковке» (отзыв автору со скринами и открытым билдом), «Тексты игры: русские ✓» (если английские — нажми, там объяснение и кнопка исправить),
  статус ИИ и «обновить список».
- **Вверху справа** — язык **RU / EN**, **«Урон считаю для скилла»** (для него считаются все цифры урона) и **«В советах важнее»**:
  урон / баланс / защита — меняет все рекомендации на всех вкладках. У обоих есть «?» с пояснением.
- **Любую карточку можно свернуть** щелчком по её заголовку (▾ / ▸); **⊟ / ⊞** справа от вкладок сворачивают
  и разворачивают все карточки вкладки. Что свёрнуто, запоминается.
...
- **«Как крафтить: от дешёвого к дорогому»** (свёрнуто) — общие принципы крафта с ценами валюты.
- **«Что улучшить в снаряжении»** — по шагам: в каком слоте какой мод заменить на какой (`было → стало`),
  что это даёт, и **шанс сделать это на надетой вещи** с вердиктом «стоит попробовать / рискованно / лотерея».
  Лотерея — значит, дешевле купить вещь с нужным модом или скрафтить с белой базы. Предметы с порчей пропускаются.
...
- **«Слоты»** (ниже) — карточка каждого надетого предмета: база, уровень, префиксы/суффиксы, пометка «с
  ...
  Внизу карточки — **«Как скрафтить с белой базы →»**: способы крафта с шансом, числом баз и ценой валюты.
...
- **Предмет против надетого** — вставь текст предмета из игры (Ctrl+Alt+C на предмете, русский клиент тоже) и увидишь разницу по
